      ...
      if replied.audio or replied.voice:
         ...
         if chat_id in QUEUE:
          ...
          try:
            ...
            await huehue.edit(f"**𝑺𝒕𝒂𝒓𝒕𝒆𝒅 𝑷𝒍𝒂𝒚𝒊𝒏𝒈** \n**NAME** : [{songname}]({link}) \n**💬 CHAT** : `{chat_id}`[𝙇𝙂𝙘𝙔・𝘼𝙇𝙀𝙓](https://t.me/lgcyalex)", disable_web_page_preview=True)
         ...
         if len(m.command) < 2:
            ...
            if search==0:
               await huehue.edit("𝑵𝒐𝒕𝒉𝒊𝒏𝒈 𝑭𝒐𝒖𝒏𝒅 𝒇𝒐𝒓 𝒕𝒉𝒆 𝑮𝒊𝒗𝒆𝒏 𝑸𝒖𝒆𝒓𝒚")
            else:
               songname = search[0]
               url = search[1]
      ...
      if hm==0:
         ...
         if chat_id in QUEUE:
            ...
            try:
               ...
               await huehue.edit(f"𝑺𝒕𝒂𝒓𝒕𝒆𝒅 𝑷𝒍𝒂𝒚𝒊𝒏𝒈 **[Radio 📻]({link})** in `{chat_id}`", disable_web_page_preview=True)
            except Exception as ep:
               await huehue.edit(f"`{ep}`")
